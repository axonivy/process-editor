...
const mockFunction: DeepPartial<Row<DataTableFeatures, Function>> = {
  original: {
    isField: true,
    name: 'ivy.cal',
    params: [],
...
test('getParentNames from mockFunction', () => {
  expect(getParentNames(mockFunction as Row<DataTableFeatures, Function>)).toEqual(['ivy.cal', 'get(String)', 'getBusinessDuration(DateTime, DateTime)']);
});
