...
test('process editor', async ({ page }) => {
  await ProcessEditor.openProcess(page, { file: 'process/quickstart.p.json' });
...
test('warning', async ({ page }) => {
  const editor = await ProcessEditor.openProcess(page, { file: 'process/screenshot/warning.p.json' });
...
test('swimlanes', async ({ page }) => {
  await ProcessEditor.openProcess(page, { file: 'process/screenshot/swimlanes.p.json', waitFor: '.pool' });
...
test('connector-process', async ({ page }) => {
  const editor = await ProcessEditor.openProcess(page, { file: 'process/market/erp.p.json', waitFor: '.start\\:callSubStart' });
